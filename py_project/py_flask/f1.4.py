...
def home() :
    ...
    # return 'homepage %s'% request.method #기본 method는 get
    return '''
        <!-- HTMl : hyper text markup language 브라우저에 쓴는 것이 주류-->
        <html>
            <head>
            ...
            <body>
                ...
                ->쿼리 결과 회원이면 ->정상 처리 (서비스 이동)
                ->회원 아니면 메시지 처리(가입,재로그인 유도)-->

                <fieldset>
                    <form action='/login' method='get'> <!--주소(url) 방식(method) 써줌
                    file:///C:/login?uid=pppppp&upw=111111111111111 요청 주소값 -->

                        <input type='text' name='uid'placeholder="아이디" autofocus required/> <!--단독 태그 name은 내부적으로 가는 값-->
                        <br/>
    ...
    '''   
# 위의 식의 결과 : http://127.0.0.1:5000/login?uid=ppp&age=10로 요청
 #이건 html 언어는 파이썬 안에서는 의미가 없는 그냥 텍스트인데, 이게 웹브라우저에서 들어가면 html문서가 해석됨 -브라우저 랜더링(html로 짜여진 것만)

@app.route('/login') #이 아이를 갈 때 데이터 전달 :name으로 된
def login():
    # request.args.get("키")
    # 전달될 데이터 획득해서 변수 담기
